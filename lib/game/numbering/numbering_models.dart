...
NumberingDifficulty difficultyForLevel(int level) {
  assert(level >= 1);
  if (level <= 20) return NumberingDifficulty.easy;
  if (level <= 40) return NumberingDifficulty.normal;
  if (level <= 80) return NumberingDifficulty.hard;
  if (level <= 120) return NumberingDifficulty.expert;
  if (level <= 160) return NumberingDifficulty.master;
