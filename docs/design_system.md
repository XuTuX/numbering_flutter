...
- 메인 챌린지 카드는 제목, 옅은 대형 퍼즐 번호, 오른쪽 아래 CTA만 둔다. 날짜 라벨과 기록 행은 넣지 않는다.
...
| Label | `10–12` | `700–800` | 꼭 필요한 상태·메타데이터만 사용 |
| Metric | `12–18` | `600–800` | 숫자를 먼저 읽을 수 있게 구성 |

- 큰 제목은 자간 `-0.5`에서 `-1.6`을 사용한다.
- 작은 라벨과 설명 문구는 기본적으로 넣지 않는다. 없어도 의미가 통하면 삭제한다.
- 허용되는 작은 라벨은 날짜, 퍼즐 번호, 실제 상태, `YOUR RANK`처럼 정보의 의미를 구분하는 경우뿐이다.
- `PLAY AT YOUR PACE` 같은 분위기용 eyebrow 문구, 태그라인, 동기부여 문구는 사용하지 않는다.
- 버튼은 카드 제목의 명사를 반복하지 않고 가장 짧은 동사로 쓴다: `Play`, `Continue`, `View`.
...
홈의 `Play` CTA는 예외적으로 검정 면을 사용하지 않는다. 챌린지 카드 배경보다 한 톤 짙은 웜그레이 tonal surface와 Ink 텍스트를 사용하며 카드 오른쪽 아래에 배치한다.
...
카드 제목만으로 목적이 명확하면 부제, eyebrow, 설명문을 추가하지 않는다.
...
홈의 Ranking 카드는 `YOUR RANK`, 현재 순위, `View Ranking`만 표시한다. 실제 비교 데이터가 없는 상승·하락 수치와 추세 화살표는 넣지 않는다.
