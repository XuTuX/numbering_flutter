...
- 메인 챌린지 카드는 날짜, 제목, CTA만 둔다. 장식용 퍼즐 번호와 기록 행은 넣지 않는다.
...
- 버튼은 카드 제목의 명사를 반복하지 않고 가장 짧은 동사로 쓴다: `Start`, `Continue`, `View`.
...
홈의 `Start` CTA는 예외적으로 검정 면을 사용하지 않는다. 챌린지 카드 배경보다 한 톤 짙은 웜그레이 tonal surface와 Ink 텍스트를 사용해 카드 안에 자연스럽게 포함시킨다.
